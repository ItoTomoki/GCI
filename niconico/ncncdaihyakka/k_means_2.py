...
def _k_init(X, n_clusters, x_squared_norms, random_state, n_local_trials=None):
    ...
    for c in range(1, n_clusters):
        ...
        for k in range(len(candidate_ids)):
            if candidate_ids[k] == n_samples:
                candidate_ids[k] = (n_samples - 1)
...
def _labels_inertia_precompute_dense(X, x_squared_norms, centers, distances):
    ...
    k = centers.shape[0]
    """
    all_distances = cosine_distances(centers, X, x_squared_norms,
                                        squared=True)
    """
    all_distances = (1 - cosine_distances(centers, X))
...
class KMeans(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def _transform(self, X):
        ...
        return (1 - cosine_distances(X, self.cluster_centers_))
