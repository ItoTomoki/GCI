...
#data = word2vec.Text8Corpus('allthreadcomments.txt')
#modelnico = word2vec.Word2Vec(data, size=100)
#modelnico.save("allthreadcomments.model")
modelnico = word2vec.Word2Vec.load("allthreadcomments.model")
model = modelnico
"""
word = [u"腹筋"]
...
def createvector(video_id):
    try:
        ...
        filename = ("comment0001/" + str(video_id) + ".txt")
        f = open(filename)
        data = f.read()
        f.close()
